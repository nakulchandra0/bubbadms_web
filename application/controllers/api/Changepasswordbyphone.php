...
class Changepasswordbyphone extends CI_Controller {
	...
	{
		verifyRequiredParams('','',array('phone','newpassword'));

		$memberdata = $this->db->get_where("memberlogin_members", array("phone"=>$_POST['phone']))->result();

		if($memberdata){

        	$data['password'] = sha1($_POST['newpassword']);

			$update = $this->ApiModel->updatedata('memberlogin_members', 'id', $memberdata[0]->id, $data);
